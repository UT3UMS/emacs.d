...
#+END_SRC
*** Direnv
Auto-activates nix-shell's env (incl. .venv) on cd/find-file
#+begin_src emacs-lisp
;; requires `direnv` binary on PATH and `direnv allow` run once per project
(use-package envrc
...
*** Python
#+begin_src emacs-lisp
;; 1. eglot for python (pyright must be on PATH inside the nix-shell/venv)
